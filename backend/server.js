...
app.post('/adduser',async(req,res)=>{
    const{f_name,l_name,emp_id,email,phone,dept,dob,role} = req.body;
    console.log(f_name,l_name,emp_id,email,phone,dept,dob,role);
    ...
    try{
        db.query('SELECT * FROM employees WHERE email=? OR emp_id=?',[email,emp_id],(err,result)=>{
            if(err){
                ...
                db.query('INSERT INTO employees(f_name,l_name,emp_id,email,phone,dept,dob,role) VALUES(?,?,?,?,?,?,?,?)',[f_name,l_name,emp_id,email,phone,dept,dob,role],(err,result)=>{
                    if(err){
...
app.listen(5000,()=>(console.log("Connected Successfully,Running 5000")));